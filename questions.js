const quizData = [
  ...
  },
  {
    ...
    "answer": "C",
    "analysis": "(A)此為反解區域檔的功能，通常使用「PTR」記錄。(B)此為NetBIOS名稱解析的範疇。(D)並非DNS正解區域檔的功能。"
  },
  {
    "id": "L4030003",
    ...
    "answer": "C",
    "analysis": "(A)這是整個軟體套件的名稱，而不是daemon。(B)這是一個獨立的守護進程，用於管理多種基於網絡的服務，而不是特定於BIND。(C)這是BIND套件的主要組件，負責處理DNS查詢和其他相關任務。(D)這不是一個標準的BIND守護進程名稱。"
  },
  {
    "id": "L4030004",
  ...
  },
  {
    ...
    "answer": "D",
    "analysis": "出現錯誤訊息「mail loops back to me (MX problem?)」通常代表郵件伺服器主機名稱與MX名稱不符。"
  },
  {
    "id": "L4040034",
  ...
  },
  {
    ...
    "answer": "D",
    "analysis": "(D)每一次網頁請求都會使得Apache透過巢狀遞迴的方式搜尋.htaccess檔案，導致Apache緩慢。雖然不少網路文章指出此降低的效能可以忽略，但仍不改效能降低的事實。"
  },
  {
    "id": "L4050011",
    "question": "若一Apache網頁伺服器禁止Directory Listing，也就是說，當一訪客造訪該Apache下網站的某目錄，該目錄沒有放置索引檔案（如：index.html）時，訪客會看到Forbidden訊息。上述「沒有索引檔案可供服務」的事件會記錄在伺服器的某一日誌檔案，請問是下列哪一個檔案？",
    "options": [
  ...
  {
    "id": "L4050014",
    "question": "Apache的dir.conf設定檔裡，有一列文字：「DirectoryIndex index.html index.cgi index.pl index.phpindex.xhtml index.htm」，其用意是當網頁瀏覽器請求的網址為「/」結尾時，Apache會在該網址請求的目標目錄裡找尋index.html、index.cgi、index.pl、index.php、index.xhtml、index.htm等檔案來回應瀏覽器。請問下列哪一個目錄放置了dir.conf設定檔？",
    "options": [
  ...
  {
    "id": "L4050015",
    "question": "如附圖所示，是網頁瀏覽器跟Apache請求一個不存在的網頁檔案時所看到的錯誤畫面，其中的訊息洩漏了Apache的版本是2.4.52，且其下的作業系統是Ubuntu。如想隱藏此資訊，可以在/etc/apache2/conf-enabled/security.conf檔案中進行設定，請問下列哪一項設定可以完成上述要求？",
  ...
  {
    "id": "L4050016",
    "question": "安裝了Apache網頁伺服器後，若想建置一網站example.com，因此撰寫了一個/etc/apache2/sites-available/example.com.conf檔案來架設一虛擬主機以服務網站example.com。在Apache重新載入設定之前，應該要執行下列哪一個指令使得Apache知道有新增一個虛擬主機？",
    "options": [
  ...
  },
  {
    ...
    "options": [
      "nodes原意是no DES，表示此openssl指令所產生的私密金鑰不要用密碼保護",
      "-days指定此openssl指令所產生的憑證之有效天數",
      "rsa:2048表示此openssl指令所產生的金鑰長度為2048位元組",
      "此openssl指令執行後，會詢問操作者一些問題，操作者的回答內容都會存入憑證"
    ],
    "answer": "C",
  ...
  {
    "id": "L4050023",
    "question": "下列哪一項設定可以將https所需之憑證（/etc/ssl/certs/apache-selfsigned.crt）指定給Apache的虛擬主機？",
    "options": [
      "SSLCertFile /etc/ssl/certs/apache-selfsigned.crt",
      "SSLCertFiles /etc/ssl/certs/apache-selfsigned.crt",
      "SSLCertificateFile /etc/ssl/certs/apache-selfsigned.crt",
      "SSLCertificateFiles /etc/ssl/certs/apache-selfsigned.crt"
    ],
    "answer": "C",
  ...
  },
  {
    ...
    "options": [
      "SSLCertKey /etc/ssl/private/apache-selfsigned.key",
      "SSLCertKeyFile /etc/ssl/private/apache-selfsigned.key",
      "SSLCertificateKey /etc/ssl/private/apache-selfsigned.key",
  ...
  {
    "id": "L4050027",
    "question": "如果Nginx網頁伺服器提供的網頁內含中文字無法正常在網頁瀏覽器顯示時，下列哪一項解法的說明正確？",
    "options": [
      "只能在server block裡加上指令AddDefaultCharset UTF-8;;",
      "只能在server block裡加上指令charset UTF-8;",
      "在server block裡或外，或是location block裡加上指令AddDefaultCharset UTF-8;",
      "在http block、server block、或是location block裡加上指令charset UTF-8;"
    ],
    "answer": "D",
    "analysis": "「AddDefaultCharset UTF-8」是Apache的解法，Nginx是使用「charset UTF-8;」。注意，整個nginx裡http block只能有一個，它寫在/etc/nginx/nginx.conf裡。"
  },
  {
    "id": "L4050028",
  ...
  {
    "id": "L4050031",
    "question": "如附圖所示，Nginx網頁伺服器所提供的一種基本的（Basic）使用者名稱和密碼認證功能，當網頁瀏覽器要瀏覽某網站或其某目錄下的資源時，會出現一個對話視窗詢問使用者帳號和密碼，需輸入正確方可開始瀏覽該目錄下的資源。網頁伺服器端需要為受保護的目錄建立合法的使用者帳號和密碼，請問下列哪一項指令可以用來建立？",
    "options": [
      "auth_file",
      "auth_basic_file",
  ...
  {
    "id": "L4050032",
    "question": "在Nginx中，若有一網站因故要改使用新的域名www.domain2.com，考量到可能有些使用者的網頁瀏覽器書籤儲存著舊的域名，或是其他網站有些網頁的超連結指向舊的域名，因此管理員不打算直接廢棄舊域名，而 是將收到的指向舊域名之網頁請求「永久」導向新的域名www.domain2.com下同樣名稱的資源。請問下列哪一項設定可以完成上述要求？",
    "options": [
